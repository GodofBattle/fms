...
$(document).ready(function() {
    ...
    $('#search-button').click(function(e) {
        ...
        setTimeout(function() {
            ...
            // by shkoh 20200730: 설비종류 선택에 따른 filter 적용
            if(g_criteria_buttongroup.selectedIndices.length > 0) {
...
/**********************************************************************************************************************************************/
/* by shkoh 20200728: resize window start                                                                                                     */
/**********************************************************************************************************************************************/
...
function calculateTreeContentHeight() {
    // by shkoh 20200728: body에서 padding-top과 padding-bottom의 크기 16을 뺀 tree content의 높이
...
function calculateEquipmentReportContentHeight() {
    // by shkoh 20200728: body에서 padding-top과 padding-bottom의 크기 16을 뺀 report grid의 높이
    const viewer_h = parseFloat(parent['mainViewer'].innerHeight || parent['mainViewer'].clientHeight) - 16;
    
    // by shkoh 20200728: '검색조건'의 높이를 계산하여 해당 부분도 뺌
    const header_h = parseFloat($('.panel-header').height());
    const header_border_h = 6;
    const header_padding_h = 11;

    // by shkoh 20200728: '검색결과'의 header의 높이를 계산하여 해당 부분을 뺌
    const panel_heading_h = parseFloat($('.panel-heading').height());
    const panel_heading_border_h = 6;
    const panel_heading_padding_h = 8;
...
/**********************************************************************************************************************************************/
/* by shkoh 20200728: resize window end                                                                                                       */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20200728: tree start                                                                                                              */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20200728: tree end                                                                                                                */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20200731: button group start                                                                                                      */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20200731: button group end                                                                                                        */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20200730: asset grid start                                                                                                        */
/**********************************************************************************************************************************************/
function initAssetGrid() {
    g_grid = $('#report-page').kendoGrid({
        ...
        columns: [
            ...
            { field: 'equip_type', title: '설비종류 | 코드', menu: false, width: 180, attributes: { 'class': 'asset_use_#:data.bUse#' }, aggregates: [ "count" ], groupHeaderTemplate: customGroupHeaderTemplate },
            { field: 'equip_model', title: '모델명', menu: false, width: 180, attributes: { 'class': 'asset_use_#:data.bUse#' }, aggregates: [ "count" ], groupHeaderTemplate: customGroupHeaderTemplate },
            { field: 'equip_id', title: '설비ID', groupable: false, menu: false, width: 60, attributes: { 'class': 'asset_use_#:data.bUse#' } },
            { field: 'equip_name', title: '설비명', groupable: false, menu: false, width: 220, attributes: { 'class': 'asset_use_#:data.bUse#' } },
            { field: 'ip', title: 'IP', menu: false, hidden: false, width: 120, attributes: { 'class': 'asset_use_#:data.bUse#' }, aggregates: [ "count" ], groupHeaderTemplate: customGroupHeaderTemplate },
            { field: 'port', title: 'Port', menu: false, hidden: false, width: 60, attributes: { 'class': 'asset_use_#:data.bUse#' }, aggregates: [ "count" ], groupHeaderTemplate: customGroupHeaderTemplate },
            { field: 'device_id', title: 'Device ID', groupable: false, menu: false, width: 80, attributes: { 'class': 'asset_use_#:data.bUse#' } },
            { field: 'description', title: '설비설명', groupable: false, sortable: false, menu: false, attributes: { 'class': 'asset_use_#:data.bUse#' } }
        ],
        ...
        dataBound: function(e) {
            // by shkoh 20200730: 데이터가 새로 읽히거나, 페이지의 이동이 될 때는 커서를 최상단으로 이동시킴
        ...
        excelExport: function(e) {
            ...
            e.workbook.sheets[0].name = 'FMS 설비정보';
            
            const column_index_index = e.sender.thead.find('th[data-field="index"]')[0].cellIndex;
            // by shkoh 20200810: 사용안함 장비의 class에 asset_use_N이 등록되어 있음으로 해당 클래스의 index만을 찾아서 사용하지 않는 장비의 값을 받아옴
            const no_used_equipment = e.sender.tbody.find('td[class="asset_use_N"]').parent().find('td[data-field="index"]');
            
            // by shkoh 20200810: export를 할 때 [순번]은 grid 생성시에 임의로 지정하기 때문에 export할 때에 순번을 강제로 넣기 위해서 data_list_index를 사용
            ...
            e.workbook.sheets[0].rows.map(function(row, idx, rows) {
                if(row.type === 'data') {
                    ...
                    // by shkoh 20200803: 사용안함 설비목록에서는 text color를 그레이로 표현하여 '사용안함' 설비로 보이게끔 수정함
...
function ipPortSort(a, b) {
    // by shkoh 20190130: IP가 존재하지 않는 경우에 '-' 로 표시하는데 해당 기호는 최상단으로 정렬됨으로 해당 데이터만 선별하여 최하단으로 정렬할 필요가 있음
...
/**********************************************************************************************************************************************/
/* by shkoh 20200730: asset grid end                                                                                                          */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20200730: inline function start                                                                                                   */
/**********************************************************************************************************************************************/
...
function exportFileName(export_ext) {
    ...
    return 'FMS설비정보목록_' + date + time + '.' + export_ext;
}
...
/**********************************************************************************************************************************************/
/* by shkoh 20200730: inline function end                                                                                                     */
/**********************************************************************************************************************************************/