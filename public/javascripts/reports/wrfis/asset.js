/**
 * by shkoh 20210416: 우리FIS 전용 자산정보 보고서 javascript
 * 
 * key id: asset
 */
...
$(function() {
    ...
    $('#search-button').on('click', function() {
        ...
        setTimeout(function() {
            ...
            // by shkoh 20210416: 조회시간을 보고서 내용에 업데이트
            updateSearchingDate();
            // by shkoh 20210416: 출력일자를 보고서에 업데이트
            updatePrintDate();
            // by shkoh 20210416: 출력자를 보고서에 업데이트
            updatePrinterName();

            const asset_list = assetDataLoad();
            Promise.all([ asset_list ]).then(function(data) {
                // by shkoh 20210416: 조회한 자산의 총 수를 입력
                updateReportAsset(data[0]);
                updateAssetData(data[0]);
            }).catch(function(err) {
                console.error(err);
            }).finally(function() {
                undisplayLoading();
...
/**********************************************************************************************************************************************/
/* by shkoh 20210416: resizing start                                                                                                          */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210416: resizing end                                                                                                            */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210416: toolbar start                                                                                                           */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210416: toolbar end                                                                                                             */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210416: datetimepicker start                                                                                                    */
/**********************************************************************************************************************************************/
...
function initDateTimePicker() {
    ...
    // by shkoh 20210503: 우리FIS 요청으로 조회시작 기간은 2009/01/01로 함
...
/**********************************************************************************************************************************************/
/* by shkoh 20210416: datetimepicker end                                                                                                      */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210416: period reset button start                                                                                               */
/**********************************************************************************************************************************************/
function initButton() {
...
/**********************************************************************************************************************************************/
/* by shkoh 20210416: period reset button end                                                                                                 */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210416: inline function start                                                                                                   */
/**********************************************************************************************************************************************/
...
function updateReportAsset(count) {
    $('#report-asset-count').text(count.length + '개');
}
...
function assetDataLoad() {
    return new Promise(function(resolve, reject) {
        $.ajax({
            async: true,
            type: 'GET',
            dataType: 'json',
            url: '/api/reports/wrfis/asset?start=' + g_searching_value.startDate + '&end=' + g_searching_value.endDate
        }).done(function(data) {
            resolve(data);
        }).fail(function(err) {
            reject(err);
...
function updateAssetData(data) {
    const asset_template = kendo.template($('#asset-template').html());

    data.forEach(function(datum, idx, arr) {
        const is_last = (idx + 1) === arr.length;
        
        const html = asset_template({
            index: idx + 1,
            object_parent_name: datum.object_parent_name,
            object_name: datum.object_name,
            object_code_name: datum.object_code_name,
            model_name: datum.model_name,
            inst_date: datum.inst_date,
            operator_name: datum.operator_name,
            manager_name: datum.manager_name,
            is_last: is_last
        });
...
/**********************************************************************************************************************************************/
/* by shkoh 20210416: inline function end                                                                                                     */
/**********************************************************************************************************************************************/