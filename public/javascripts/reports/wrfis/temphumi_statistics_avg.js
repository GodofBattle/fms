/**
 * by shkoh 20210812: 우리FIS 전용 온습도 가동현황 보고서 javascripts
 * 
 * key id: temphumi_statistics_avg
 */
...
/**
 * by shkoh 20210812: 보고서 Export 정의
 */
 const report_config = {
    // by shkoh 20210812: 보고서 형태가 가로방향(landscape)인지 세로방향(portrait)인지 여부
    is_landscape: false,
     
    // by shkoh 20210812: 보고서 디자인 시 작성한 colgroup의 수
    columns: {
        count: $('col').length,
    },
    pdf: {
        // by shkoh 20210812: PDF Export 시, 기본 font의 크기
        defaultFontSize: 7
    },
    excel: {
        columnWidth: 30,
        // by shkoh 20210812: Excel Export 시, 엑셀 시트의 셀 Column의 전체 수
        // by shkoh 20210812: 보고서 전체 사용 column 수에서 좌우 여백를 고려하여, (보고서에서 사용한 <colgroup> 수) + 2
...
$(function() {
    ...
    $('#search-button').click(function() {
        setTimeout(function() {
            ...
            $('#th-contents-body').empty();

            // by shkoh 20210812: summery 초기화
            g_summery = {
                temp: 0,
                temp_count: 0,
                humi: 0,
            ...
            // by shkoh 20210812: 보고서 생성 시 사용한 통계 데이터의 명칭을 화면에 업데이트
            updateReportTable();
            // by shkoh 20210812: 조회기간을 보고서 내용에 업데이트
            updateSearchingDate();
            // by shkoh 20210812: 출력일자를 보고서 내용에 업데이트
            updatePrintDate();
            // by shkoh 20210812: 출력자를 보고서 내용에 업데이트
            updatePrinterName();

            const th_list = thDataLoad();
            Promise.all([ th_list ]).then(function(data) {
                updateTHData(data[0]);
            }).then(function() {
                updateSummery();
            }).catch(function(err) {
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: resizing start                                                                                                          */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: resizing end                                                                                                            */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: toolbar start                                                                                                           */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: toolbar end                                                                                                             */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: table dropdown list start                                                                                               */
/**********************************************************************************************************************************************/
function initTableDropDownList() {
    g_table_inst = $('#table-picker').kendoDropDownList({
        ...
        change: function(e) {
            ...
            let report_title = (this.value() === 'year') ? '온습도 가동현황 연간 보고서' : (this.value() === 'month') ? '온습도 가동현황 월간 보고서' : '온습도 가동현황 일간 보고서';
    ...
    let report_title = (g_table_inst.value() === 'year') ? '온습도 가동현황 연간 보고서' : (g_table_inst.value() === 'month') ? '온습도 가동현황 월간 보고서' : '온습도 가동현황 일간 보고서';
    $('#report-title').text(report_title);
}
/**********************************************************************************************************************************************/
/* by shkoh 20210812: table dropdown list end                                                                                                 */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: datetimepicker start                                                                                                    */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: datetimepicker end                                                                                                      */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: period reset button start                                                                                               */
/**********************************************************************************************************************************************/
function initButton() {
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: period reset button end                                                                                                 */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: inline function start                                                                                                   */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: inline function end                                                                                                     */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: inline function - reporting start                                                                                       */
/**********************************************************************************************************************************************/
...
/**********************************************************************************************************************************************/
/* by shkoh 20210812: inline function - reporting end                                                                                         */
/**********************************************************************************************************************************************/

/**********************************************************************************************************************************************/
/* by shkoh 20210812: inline function - pue data start                                                                                        */
/**********************************************************************************************************************************************/
function thDataLoad() {
    return new Promise(function(resolve, reject) {
        $.ajax({
            ...
            type: 'GET',
            dataType: 'json',
            url: '/api/reports/wrfis/thstatavg?table=' + g_searching_value.table + '&start=' + g_searching_value.date
        }).done(function(data) {
            resolve(data);
        }).fail(function(err) {
            reject(err);
...
function updateTHData(data) {
    ...
    for(let idx = 1; idx <= last_idx; idx++) {
        ...
        const html = th_template({
            index: idx,
            period: datum ? kendo.toString(new Date(datum.period), period_format) : '',
            avg_temp: convertTemp(datum ? datum.avg_temp : undefined),
            avg_humi: convertHumi(datum ? datum.avg_humi : undefined),
            is_last: idx === last_idx
        });

        $('#th-contents-body').append(html);
    }
}
...
function updateSummery() {
    if(g_summery.temp_count === 0) {
        ...
        $('#avg_temp').text(kendo.toString(avg_temp, 'n1'));
    }
    ...
    if(g_summery.humi_count === 0) {
        ...
        $('#avg_humi').text(kendo.toString(avg_humi, 'n1'));
    }
}
/**********************************************************************************************************************************************/
/* by shkoh 20210812: inline function - pue data end                                                                                          */
/**********************************************************************************************************************************************/
...
function createPDFRows() {
    ...
    // by shkoh 20210419: 자산정보 보고서 본 내용
    const contents_tr = $('#th-contents-body, .r-table.r-table-contents > tfoot').find('tr');
    g_export_util.CreatePDFBody(contents_tr, rows, report_config.pdf.defaultFontSize);
