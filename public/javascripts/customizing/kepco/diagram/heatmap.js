const HeatMap = function(_id, _options) {
    ...
    function createHeatmap() {
        ...
        const max_radius = Math.max($('#map').width(), $('#map').height());
    ...
    function setHeatmapData(items) {
        const temp_point = items.map(function(item) {
            ...
            return [
                parseFloat(item.pos_x) * parseFloat($('#map').width()),
                parseFloat(item.pos_y) * parseFloat($('#map').height()),
                val
            ];
        });
        ...
        const x_step = $('#map').width() / point_number;
        const y_step = $('#map').height() / point_number;
        ...
        for(let x_idx = 0; x_idx < point_number; x_idx++) {    
            for(let y_idx = 0; y_idx < point_number; y_idx++) {    
                if(x_idx === 0 || y_idx === 0) {
                    ...
                    temp_point.push([$('#map').width() - position_interval, $('#map').height() - position_interval, 0]);
                } else if(x_idx === point_number - 1) {
                    temp_point.push([$('#map').width() - position_interval, y, 0]);
                } else if(y_idx === point_number - 1) {
                    temp_point.push([x, $('#map').height() - position_interval, 0]);
                }
