...
$(function() {
    ...
    checkEquipSettings($('#tab-panel').data('kendoTabStrip').contentElements[0]);
});
...
function initTabPanel() {
    $('#tab-panel').kendoTabStrip({
        ...
        animation: false,
        activate: function(e) {
            g_wemb_mapping_data.page_name = e.item.id;
...
function initLinkerButton() {
    ...
    $('.custom-link-set').on('click', function() {
        g_wemb_mapping_data.object_name = $(this).parents('li').attr('data');

        const _title = $(this).parents('.list-group').children('.custom-list-title').children('span').text();
        g_wemb_mapping_data.description = _title + (_title.length > 0 ? ' ' : '') + $(this).parent().parent().children('span.obj-name').text();
...
function initModal() {
    $('#modalDialogTree').on('show.bs.modal', function() {
        $('#modalTreeTitle').text('WEMB 연결설비 설정 - ' + g_wemb_mapping_data.description);
    ...
    $('#modalDialogTree').on('hide.bs.modal', function() {
        $('#modalTreeTitle').text('WEMB 연결설비 설정');
...
function onCheckTreeNode(event, treeId, treeNode) {
    ...
    $('#modalTreeTitle').text('WEMB 연결설비 설정 - ' + g_wemb_mapping_data.description + ': 항목 ' + checked_count + '개 선택됨');
}
...
function importItems() {
    $.ajax({
        ...
        dataType: 'json',
        cache: false,
        url: '/api/wrfis/wemb/mapper?pagename=' + g_wemb_mapping_data.page_name + '&objectname=' + g_wemb_mapping_data.object_name
    }).done(function(data) {
...
function checkEquipSettings(ele) {
    $.ajax({
        ...
        dataType: 'json',
        cache: false,
        url: '/api/wrfis/wemb/mapper?pagename=' + g_wemb_mapping_data.page_name
    }).done(function(data) {
...
function applyItems(clear) {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        url: '/api/wrfis/wemb/item',
        data: g_wemb_mapping_data
    }).done(function() {
        alert('설비연결 정보 저장성공');
...
function getCheckedTreeNode(clear) {
    ...
    g_wemb_mapping_data.group_ids = g_ids.toString();
    g_wemb_mapping_data.equip_ids = e_ids.toString();
    g_wemb_mapping_data.sensor_ids = s_ids.toString();
    
    if(clear) g_wemb_mapping_data.description = '';
}
...
function popupSet() {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        url: '/api/wrfis/wemb/itempopup',
        data: {
            page_name: g_wemb_mapping_data.page_name,
            object_name: g_wemb_mapping_data.object_name,
            popup_equip_id: id
        }
    }).done(function() {
        alert('팝업설정 완료');
